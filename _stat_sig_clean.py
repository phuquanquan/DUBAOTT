"""
Statistical Significance Test - No LOG writes, just print.
"""
...
CHECKPOINT = Path("agents/_stat_cp.json")
...
def main() -> None:
    ...
    print("=== Fast Stat Sig Test ===", flush=True)
    ...
    print(f"Data: {len(results)} rows, {len(test_indices)} test days", flush=True)
    print(f"Baseline: {p0*100:.2f}%, top_k={top_k}", flush=True)
    ...
    if CHECKPOINT.exists():
        ...
        print(f"Resuming: {len(cp['done'])} done", flush=True)
    ...
    for sig_name in all_names:
        ...
        print(f"  {sig_name}: Hit={srate*100:.2f}% Delta={delta:+.2f}pp "
              f"binom={binom_p:.4f}{sig_b} perm={perm_p:.4f}{sig_p} ({time.time()-t_sig:.0f}s)", flush=True)
    ...
    print(f"\n{'='*90}", flush=True)
    print(f"[SUMMARY] {len(cp['results'])} signals, {len(test_indices)} test days, {time.time()-t_start:.0f}s", flush=True)
    print(f"  Binomial sig: {len(sig_binom)}, Permutation sig: {len(sig_perm)}, Both: {len(sig_both)}", flush=True)
    ...
    for r in cp["results"]:
        ...
        print(f"  {r['signal']:<32} Hit={r['hit_rate_pct']:5.2f}% Delta={r['delta_pct']:+6.2f}pp "
              ...
              f"CI=[{r['ci_lower_pct']:.1f},{r['ci_upper_pct']:.1f}]", flush=True)
