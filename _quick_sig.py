"""Quick test - first 3 signals only."""
...
LOG = Path("agents/_quick_log.txt")
...
log(f"Cache: {len(freq_cache)} entries")
...
# Test hot_trend only
log("Testing hot_trend...")
t0 = time.time()
sig_hits, freq_hits_list = [], []
for day_idx, split_idx in enumerate(test_indices[:10]):
    train = list(results[:split_idx])
    actual = actual_targets(results[split_idx], "loto2")
    sig_ranked = rank_candidates(train, "hot_trend", top_k, width)
    ...
    log(f"  day {day_idx}: hit={int(hit)}, fhit={int(fhit)}")
log(f"hot_trend 10 days: {time.time()-t0:.1f}s")

# Test cold_return only
log("Testing cold_return...")
t0 = time.time()
sig_hits2 = []
for day_idx, split_idx in enumerate(test_indices[:10]):
    train = list(results[:split_idx])
    actual = actual_targets(results[split_idx], "loto2")
    sig_ranked = rank_candidates(train, "cold_return", top_k, width)
    freq_ranked = freq_cache[day_idx]
    pred = [c for c, _ in sig_ranked]
    freq_pred = [c for c, _ in freq_ranked]
    hit, _, _, _ = evaluate_prediction_set(pred, actual, universe)
    sig_hits2.append(int(hit))
    log(f"  day {day_idx}: hit={int(hit)}")
log(f"cold_return 10 days: {time.time()-t0:.1f}s")

# Full hot_trend
log("Full hot_trend 587 days...")
t0 = time.time()
sig_hits = []
for day_idx, split_idx in enumerate(test_indices):
    ...
    sig_hits.append(int(hit))
    if day_idx % 100 == 0:
        log(f"  day {day_idx}/{len(test_indices)}")
log(f"Full hot_trend: {time.time()-t0:.1f}s, hits={sum(sig_hits)}/{len(sig_hits)}")
log("ALL DONE")
